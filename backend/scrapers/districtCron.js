// Persistent District scheduler - runs the same logic as runDistrictScraper.js
// but stays alive and repeats every SCRAPE_INTERVAL_HOURS, instead of running
// once and exiting. Kept as its own long-running process (not merged into
// server.js's cron) since importing District through that shared chain was
// the exact context where it mysteriously stopped working before.
//
// Run this in its own terminal, alongside `npm run dev` and the frontend:
//   npm run cron:district
...
async function runOnce() {
  console.log(`\n[${new Date().toISOString()}] Starting District scrape...`);
  let allEvents = [];
  ...
  console.log(`Total District events: ${allEvents.length}`);
...
async function start() {
  ...
  console.log('✓ Connected to MongoDB (District cron)');

  const intervalHours = Number(process.env.SCRAPE_INTERVAL_HOURS) || 6;
  console.log(`✓ District will scrape now, then every ${intervalHours} hours`);
...
start().catch(err => {
  console.error('District cron failed to start:', err);
  process.exit(1);
});

process.on('SIGINT', async () => {
  console.log('\nStopping District cron...');
